    ...
    <nav class="flex-1 px-3 py-4 space-y-1">
        ...
        <a href="{{ route('activemember.posts.create') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all
                  {{ $activePage === 'sections' ? 'nav-link-active' : 'text-on-surface-variant hover:bg-surface-container' }}">
            <span class="material-symbols-outlined text-xl">dashboard</span>
            Create Post
        </a>

        <a href="{{ route('activemember.feed') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all
                  {{ $activePage === 'users' ? 'nav-link-active' : 'text-on-surface-variant hover:bg-surface-container' }}">
            <span class="material-symbols-outlined text-xl">post</span>
            Feeds
        </a>
